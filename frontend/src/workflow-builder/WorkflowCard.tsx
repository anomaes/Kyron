...
export function WorkflowCard({ data, selected }: NodeProps<BuilderNode>) {
  ...
  return <div className={`builder-node type-${data.type} ${selected ? "selected" : ""}`}><Handle type="target" position={Position.Left} /><div className="builder-node-head"><span>{icons[data.type]}</span><small>{data.type.replaceAll("_", " ")}</small>{data.workflowNode.join === "or" && <b>OR</b>}</div><strong>{data.label}</strong><p>{preview}</p><Handle type="source" position={Position.Right} /></div>;
}
