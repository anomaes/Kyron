...
function sourceLabel(state: PiModelsAdminState): string {
  if (state.source === "database") return `Database revision ${state.active_version}`;
  if (state.source === "file") return "Optional file bootstrap";
  return "Pi built-in catalog";
}
...
export function PiModelsAdminPanel() {
  ...
  const save = useMutation({
    ...
    onSuccess: async (result) => { setEditorOpen(false); setValidation(null); setNotice(`Revision ${result.active_version} is active for new runs`); await refresh(); },
  });
  const activate = useMutation({
    mutationFn: (id: string) => api<PiModelsAdminState>(`/admin/pi-models/revisions/${id}/activate`, { method: "POST" }),
    onSuccess: async (result) => { setNotice(`Revision ${result.active_version} restored for new runs`); await refresh(); },
  });
  const deactivate = useMutation({
    mutationFn: () => api<void>("/admin/pi-models/active", { method: "DELETE" }),
    onSuccess: async () => { setNotice("Database configuration disabled; fallback is now active"); await refresh(); },
  });
  ...
  return <>
    <div className="provider-overview">
      <section className="provider-status-card">
        <div><span className="provider-orbit">π</span><div><p className="eyebrow">Active source</p><h2>{sourceLabel(state)}</h2><p>{state.providers.length ? `${state.providers.length} custom provider${state.providers.length === 1 ? "" : "s"} available alongside Pi's built-ins.` : "Workflows can use Pi's built-in providers. Add a custom endpoint when needed."}</p></div></div>
        <div className="provider-status-actions"><button onClick={() => openEditor()}>Configure providers</button>{state.source === "database" && <button className="secondary" disabled={deactivate.isPending} onClick={() => deactivate.mutate()}>Use fallback</button>}</div>
      </section>
    ...
    <section className="panel provider-panel">
      <div className="panel-title"><h2>Configured endpoints</h2><span>{state.source.toUpperCase()}</span></div>
    ...
    <section className="panel provider-history">
      <div className="panel-title"><h2>Configuration history</h2><span>{state.revisions.length} REVISION{state.revisions.length === 1 ? "" : "S"}</span></div>
      {state.revisions.length ? state.revisions.map((revision) => <div className="provider-revision" key={revision.id}><div><strong>Revision {revision.version}</strong>{revision.active && <span className="badge success">ACTIVE</span>}<small>{new Date(revision.created_at).toLocaleString()} · {revision.providers.map((item) => item.id).join(", ") || "Built-ins only"}</small></div>{!revision.active && <button className="secondary" disabled={activate.isPending} onClick={() => activate.mutate(revision.id)}>Restore</button>}</div>) : <p className="provider-history-empty">Saved revisions will appear here and can be restored without editing JSON.</p>}
    </section>
    ...
    {editorOpen && <div className="modal-backdrop" onMouseDown={() => setEditorOpen(false)}><div className="modal provider-editor-modal" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <ProviderBuilder document={parsed} selectedProvider={selectedProvider} onSelect={setSelectedProvider} onChange={(document) => { setEditorText(JSON.stringify(document, null, 2)); setBuilderError(null); setValidation(null); }} onError={setBuilderError} />
      ...
      <footer><button className="secondary" onClick={() => setEditorOpen(false)}>Cancel</button><button className="secondary" disabled={!parsed || validate.isPending} onClick={() => parsed && validate.mutate(parsed)}>{validate.isPending ? "Validating…" : "Validate"}</button><button disabled={!parsed || save.isPending} onClick={() => parsed && save.mutate(parsed)}>{save.isPending ? "Activating…" : "Save & activate"}</button></footer>
    </div></div>}
...
function ProviderBuilder({ document, selectedProvider, onSelect, onChange, onError }: {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!document) return onError("Fix the advanced JSON before using the guided editor");
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(id)) return onError("Provider ID must use letters, numbers, dots, underscores, or hyphens");
    if ((bearer && !CREDENTIAL_PATTERN.test(bearer)) || (xKey && !CREDENTIAL_PATTERN.test(xKey))) return onError("Credential names must be valid environment variable names");
  ...
  return <div className="provider-builder">
    ...
    <form key={selectedProvider || "new"} onSubmit={submit}>
      ...
      <div className="provider-builder-actions">{selectedProvider && <button className="danger" type="button" onClick={remove}>Remove provider</button>}<button type="submit">Apply to draft</button></div>
    </form>
  </div>;
}
