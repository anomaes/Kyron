...
import { GateResponseControls } from "../components/GateResponseControls";
...
function buildExpandedGraph(graph: RunGraph): { nodes: Node[]; edges: Edge[] } {
  ...
  if (rootInvocation && rootDefinition) {
    for (const workflowNode of rootDefinition.nodes) {
      ...
      const piSelection: PiNodeSelection | undefined = workflowNode.type === "prompt" && execution ? { executionId: execution.id, label: workflowNode.label, nodePath: execution.node_path, status: execution.status, currentAttempt: execution.current_attempt } : undefined;
      flowNodes.push({ id, position: workflowNode.position, sourcePosition: Position.Right, targetPosition: Position.Left, data: { label: <RunNodeLabel type={workflowNode.type} label={workflowNode.label} status={execution?.status ?? "PENDING"} />, piSelection }, className: `status-${(execution?.status ?? "PENDING").toLowerCase()} ${piSelection ? "pi-selectable-node" : ""}`, draggable: false, ariaLabel: piSelection ? `Open Pi activity for ${workflowNode.label}` : workflowNode.label });
    }
  ...
  for (let depth = 1; depth <= maxDepth; depth += 1) {
    ...
    for (const invocation of row) {
      ...
      for (const workflowNode of definition.nodes) {
        ...
        const piSelection: PiNodeSelection | undefined = workflowNode.type === "prompt" && execution ? { executionId: execution.id, label: workflowNode.label, nodePath: execution.node_path, status: execution.status, currentAttempt: execution.current_attempt } : undefined;
        flowNodes.push({ id, position, sourcePosition: Position.Right, targetPosition: Position.Left, data: { label: <RunNodeLabel type={workflowNode.type} label={workflowNode.label} status={execution?.status ?? "PENDING"} />, piSelection }, className: `status-${(execution?.status ?? "PENDING").toLowerCase()} ${piSelection ? "pi-selectable-node" : ""}`, draggable: false, ariaLabel: piSelection ? `Open Pi activity for ${workflowNode.label}` : workflowNode.label });
      }
...
type WaveGroup = { invocationId: string; workflowName: string; invocationPath: string; waves: RunGraph["waves"] };

function groupWavesByInvocation(graph: RunGraph): WaveGroup[] {
  const invocations = new Map(graph.invocations.map((invocation) => [invocation.id, invocation]));
  const groups = new Map<string, WaveGroup>();
  for (const wave of graph.waves) {
    const invocation = invocations.get(wave.invocation_id);
    const group = groups.get(wave.invocation_id) ?? {
      invocationId: wave.invocation_id,
      workflowName: invocation ? graph.snapshot.workflows[invocation.workflow_id]?.name ?? invocation.workflow_id : "Unknown invocation",
      invocationPath: invocation?.invocation_path ?? "—",
      waves: [],
    };
    group.waves.push(wave);
    groups.set(wave.invocation_id, group);
  }
  const firstStart = (group: WaveGroup) => Math.min(...group.waves.map((wave) => wave.started_at ? Date.parse(wave.started_at) : Number.MAX_SAFE_INTEGER));
  for (const group of groups.values()) group.waves.sort((left, right) => left.wave_index - right.wave_index);
  return [...groups.values()].sort((left, right) => firstStart(left) - firstStart(right) || left.invocationPath.localeCompare(right.invocationPath));
}
...
export function RunDetailPage() {
  ...
  const [fullscreenPanel, setFullscreenPanel] = useState<"graph" | "logs" | "activity" | null>(null);
  const [selectedPiNode, setSelectedPiNode] = useState<PiNodeSelection | null>(null);
  const [followingLogs, setFollowingLogs] = useState(true);
  const terminalRef = useRef<HTMLDivElement>(null);
  const followLogsRef = useRef(true);
  const action = useMutation({ mutationFn: ({ path, body }: { path: string; body?: unknown }) => api(`/runs/${runId}/${path}`, body ? json("POST", body) : { method: "POST" }), onSuccess: async () => { setFeedback(""); await Promise.all([client.invalidateQueries({ queryKey: ["run", runId] }), client.invalidateQueries({ queryKey: ["run-graph", runId] }), client.invalidateQueries({ queryKey: ["run-report", runId] })]); } });
  ...
  const waveGroups = useMemo(() => graph.data ? groupWavesByInvocation(graph.data) : [], [graph.data]);
  const currentPiNode = useMemo(() => {
    if (!selectedPiNode) return null;
    const execution = graph.data?.nodes.find((node) => node.id === selectedPiNode.executionId);
    return execution ? { ...selectedPiNode, nodePath: execution.node_path, status: execution.status, currentAttempt: execution.current_attempt } : selectedPiNode;
  }, [graph.data?.nodes, selectedPiNode]);
  const runLogEvents = useMemo(() => logs.filter((event) => {
    if (event.type !== "pi_event") return true;
    return ["tool_start", "tool_end", "lifecycle", "error"].includes(event.event?.kind ?? "");
  }), [logs]);
  ...
  const openGates = graph.data?.gates.filter((gate) => gate.status === "OPEN") ?? [];
  useLayoutEffect(() => {
    if (!followLogsRef.current || !terminalRef.current) return;
    terminalRef.current.scrollTop = terminalRef.current.scrollHeight;
  }, [runLogEvents.length, fullscreenPanel]);
  useEffect(() => {
  ...
  return <section>{deleteOpen && <div className="modal-backdrop" onMouseDown={() => setDeleteOpen(false)}><div className="modal confirm-modal" onMouseDown={(event) => event.stopPropagation()}><span className="danger-mark">×</span><h2>Delete this run?</h2><p>This permanently removes the run's worktree, local branch, stored output, logs, report, and execution history. It does not remove a remote branch or pull/merge request.</p>{remove.error && <p className="error">{remove.error.message}</p>}<footer><button type="button" className="secondary" onClick={() => setDeleteOpen(false)}>Cancel</button><button type="button" className="danger" disabled={remove.isPending} onClick={() => remove.mutate()}>{remove.isPending ? "Deleting…" : "Delete run"}</button></footer></div></div>}{run.data && <><header className="run-header"><div><Link to="/runs" className="eyebrow">← All runs</Link><h1>{run.data.root_workflow_id}</h1><div className="run-meta"><StatusBadge status={run.data.status} />{run.data.local_definition_test && <span className="review-badge">Local definition test</span>}<code>{run.data.id}</code></div></div><div className="run-actions">{run.data.change_request_url && <a className="button secondary" href={run.data.change_request_url} target="_blank" rel="noreferrer">Open {run.data.reviewer_provider === "github" ? "PR" : "MR"} ↗</a>}{canOperateRun && ["FAILED", "INTERRUPTED", "CANCELLED"].includes(run.data.status) && <button disabled={action.isPending} onClick={() => action.mutate({ path: "resume" })}>{action.isPending && action.variables?.path === "resume" ? "Resuming…" : "Resume"}</button>}{canOperateRun && ["QUEUED", "RUNNING", "RESUMING", "AWAITING_FEEDBACK"].includes(run.data.status) && <button className="danger" disabled={action.isPending} onClick={() => action.mutate({ path: "cancel" })}>{action.isPending && action.variables?.path === "cancel" ? "Cancelling…" : "Cancel"}</button>}{canDeleteRun && <button className="danger" onClick={() => setDeleteOpen(true)}>Delete run</button>}</div></header>{action.error && <div className="error action-error">{action.error.message}</div>}<RunErrorBanner errorType={run.data.error_type} errorMessage={run.data.error_message} /><div className="run-summary"><div><span>Base</span><strong>{run.data.base_ref}</strong><code>{run.data.base_commit_sha.slice(0, 12)}</code></div><div><span>Run branch</span><strong className="mono">{run.data.branch_name ?? "Preparing…"}</strong></div><div><span>Current HEAD</span><code>{run.data.current_head_sha?.slice(0, 12) ?? "—"}</code></div><div><span>Started</span><strong>{run.data.started_at ? new Date(run.data.started_at).toLocaleString() : "Queued"}</strong></div><RunUsageSummary data={usage.data} loading={usage.isLoading} error={usage.isError} /></div>{run.data.local_definition_test && <div className="feedback-banner"><div><p className="eyebrow">Local-only run</p><h2>Code-host push is disabled</h2><p>This run tests the exact local definition snapshot. Its worktree and results stay on the Kyron host.</p></div></div>}</>}
    {openGates.map((gate) => {
      const canControl = gate.eligible_snapshot.requirements.some((requirement) => requirement.users.some((actor) => actor.provider === user?.provider && actor.provider_user_id === user?.provider_user_id));
      const canOverride = Boolean(access.data?.permissions.includes("gate.override"));
      const approvalCount = graph.data?.gate_decisions.filter((decision) => decision.gate_instance_id === gate.id && decision.event_type === "approval" && !decision.superseded).length ?? 0;
      const invocation = graph.data?.invocations.find((item) => item.id === gate.invocation_id);
      const feedbackPath = `gates/${gate.id}/feedback`;
      const approvalPath = `gates/${gate.id}/approve`;
      const overridePath = `gates/${gate.id}/override`;
      const pendingPath = action.isPending ? action.variables?.path : undefined;
      const pendingAction = pendingPath === feedbackPath ? "feedback" : pendingPath === approvalPath ? "approval" : pendingPath === overridePath ? "override" : null;
      return <div className={`feedback-banner gate-banner ${canControl || canOverride ? "" : "gate-banner-readonly"}`} key={gate.id}><div className="gate-banner-copy"><p className="eyebrow">Awaiting review</p><h2>{gate.policy_snapshot.name ?? "Review requested"}</h2><p>{canControl ? "Approve this checkpoint or send it back with clear revision notes." : "This checkpoint is waiting for its configured approval policy."}</p><div className="gate-meta">{invocation?.invocation_path && <code>{invocation.invocation_path}</code>}<span>Commit</span><code>{gate.checkpoint_commit_sha.slice(0, 12)}</code>{approvalCount > 0 && <span className="approval-count">{approvalCount} approval{approvalCount === 1 ? "" : "s"}</span>}</div></div><GateResponseControls feedback={feedback} canControl={canControl} canOverride={canOverride} isBusy={action.isPending} pendingAction={pendingAction} onFeedbackChange={setFeedback} onSubmitFeedback={() => action.mutate({ path: feedbackPath, body: { message: feedback } })} onApprove={() => action.mutate({ path: approvalPath })} onOverride={() => { const reason = window.prompt("Reason for overriding this gate"); if (reason) action.mutate({ path: overridePath, body: { reason } }); }} /></div>;
    })}
    <div className="run-layout"><div className={`panel graph-panel ${fullscreenPanel === "graph" ? "panel-fullscreen" : ""}`}><div className="panel-title"><h2>Workflow graph</h2><div className="panel-title-actions"><span>{graph.data?.invocations.length ?? 0} invocation{graph.data?.invocations.length === 1 ? "" : "s"} · {graph.data?.nodes.length ?? 0} executions</span><button type="button" className="panel-expand-button" aria-label={fullscreenPanel === "graph" ? "Exit workflow graph fullscreen" : "Show workflow graph fullscreen"} title={fullscreenPanel === "graph" ? "Exit fullscreen (Esc)" : "Fullscreen"} onClick={() => setFullscreenPanel(fullscreenPanel === "graph" ? null : "graph")}>{fullscreenPanel === "graph" ? "Close" : "Fullscreen"}</button></div></div><div className="flow-canvas expanded"><ReactFlow key={fullscreenPanel === "graph" ? "fullscreen" : "inline"} nodes={expandedGraph.nodes} edges={expandedGraph.edges} fitView nodesDraggable={false} nodesConnectable={false} onNodeClick={(_, node) => { const selection = node.data.piSelection as PiNodeSelection | undefined; if (selection) { setSelectedPiNode(selection); setFullscreenPanel(null); } }}><Background /><Controls showInteractive={false} /></ReactFlow></div></div>{currentPiNode ? <PiActivityPanel runId={runId} node={currentPiNode} attempts={graph.data?.attempts ?? []} liveEvents={logs} connectionState={logConnectionState} fullscreen={fullscreenPanel === "activity"} onShowRunLog={() => { setSelectedPiNode(null); if (fullscreenPanel === "activity") setFullscreenPanel(null); }} onToggleFullscreen={() => setFullscreenPanel(fullscreenPanel === "activity" ? null : "activity")} /> : <div className={`panel log-panel ${fullscreenPanel === "logs" ? "panel-fullscreen" : ""}`}><div className="panel-title"><div className="activity-tabs"><button type="button" className="active">Run log</button></div><div className="panel-title-actions"><span className={`live-dot ${logConnectionState}`}>{logConnectionState}</span>{!followingLogs && runLogEvents.length > 0 && <button type="button" className="log-latest-button" onClick={jumpToLatestLog}>Latest ↓</button>}<button type="button" className="panel-expand-button" aria-label={fullscreenPanel === "logs" ? "Exit live stream fullscreen" : "Show live stream fullscreen"} title={fullscreenPanel === "logs" ? "Exit fullscreen (Esc)" : "Fullscreen"} onClick={() => setFullscreenPanel(fullscreenPanel === "logs" ? null : "logs")}>{fullscreenPanel === "logs" ? "Close" : "Fullscreen"}</button></div></div><div className="terminal" ref={terminalRef} onScroll={handleLogScroll}>{runLogEvents.length ? runLogEvents.map((event, index) => { const path = event.node_path ?? event.invocation_path; const source = event.source ?? event.event_type ?? event.level ?? event.type; return <div className="log-line" key={`${event.sequence ?? event.event?.event_index ?? index}-${index}`}><time>{event.timestamp?.slice(11, 19) ?? ""}</time><span className={`log-source ${event.source ?? event.level?.toLowerCase()}`} title={source}>{source}</span><pre>{path && <span className="log-path">{path}</span>}{event.line ?? event.message ?? event.event_type}</pre></div>; }) : <div className="terminal-empty">{logConnectionState === "retrying" ? "Live connection unavailable; retrying and polling engine logs…" : logConnectionState === "complete" ? "No engine output was recorded for this run." : "Waiting for engine output…"}</div>}</div></div>}</div>
    <div className="panel wave-panel"><div className="panel-title"><h2>Execution waves</h2><span>Git checkpoint boundaries</span></div>{waveGroups.length ? <table><thead><tr><th>Wave</th><th>Status</th><th>Start SHA</th><th>End SHA</th><th>Started</th></tr></thead>{waveGroups.map((group) => <tbody key={group.invocationId}><tr className="wave-group"><td colSpan={5}><div><span><strong>{group.workflowName}</strong><code>{group.invocationPath}</code></span><small>{group.waves.length} wave{group.waves.length === 1 ? "" : "s"}</small></div></td></tr>{group.waves.map((wave) => <tr key={wave.id}><td>#{wave.wave_index}</td><td><StatusBadge status={wave.status} /></td><td><code>{wave.start_commit_sha.slice(0, 10)}</code></td><td><code>{wave.end_commit_sha ? wave.end_commit_sha.slice(0, 10) : "—"}</code></td><td>{wave.started_at ? new Date(wave.started_at).toLocaleString() : "—"}</td></tr>)}</tbody>)}</table> : <p className="wave-empty">No execution waves have been recorded yet.</p>}</div>
    <div className="panel report-panel"><div className="panel-title"><h2>Traceability report</h2><div className="panel-title-actions"><span>{report.data?.frozen ? "Final execution record" : "Live audit record"}</span><a className="report-export-button" href={`/api/runs/${runId}/report/export`} download>Export report</a></div></div><div className="report-summary"><div><span>Triggered by</span><strong>{String((report.data?.run.triggered_by as Record<string, unknown> | undefined)?.display_name ?? "—")}</strong></div><div><span>Definition</span><code>{String(report.data?.run.workflow_definition_commit_sha ?? "—").slice(0, 12)}</code></div><div><span>Gates</span><strong>{report.data?.gates.length ?? 0}</strong></div><div><span>Result</span><StatusBadge status={String(report.data?.run.status ?? "PENDING")} /></div></div>{report.data?.gates.map((gate) => <article className="report-gate" key={gate.id}><header><div><small>{gate.invocation_path}</small><h3>{gate.policy_snapshot.name} · {gate.node_id}</h3></div><StatusBadge status={gate.status} /></header><p>Checkpoint <code>{gate.checkpoint_commit_sha.slice(0, 12)}</code> · opened {new Date(gate.opened_at).toLocaleString()}</p>{gate.policy_snapshot.requirements.map((requirement) => <div className="report-requirement" key={requirement.key}><strong>{requirement.name}</strong><span>Quorum {requirement.quorum}</span></div>)}{gate.decisions.map((decision) => <div className={`report-decision ${decision.superseded ? "superseded" : ""}`} key={decision.id}><strong>{decision.event_type}</strong><span>{decision.actor_snapshot.display_name ?? decision.actor_snapshot.provider_username}</span><time>{new Date(decision.created_at).toLocaleString()}</time>{decision.message && <p>{decision.message}</p>}</div>)}</article>)}{report.data?.post_run_lifecycle.map((event, index) => <p className="lifecycle-event" key={index}>Post-run: {String(event.event_type)} by @{String(event.actor_username)}</p>)}</div>
  </section>;
}
