...
function buildExpandedGraph(graph: RunGraph): { nodes: Node[]; edges: Edge[] } {
  ...
  for (const invocation of invocations) {
    ...
    if (!isRoot) {
      ...
      flowNodes.push({ id: headerId, position: { x: offsetX, y: offsetY }, sourcePosition: Position.Right, targetPosition: Position.Left, data: { label: headerLabel }, className: "invocation-summary", draggable: false });
      const siblings = childrenByParent.get(invocation.parent_node_execution_id ?? "") ?? [];
    ...
    for (const workflowNode of definition.nodes) {
      ...
      flowNodes.push({ id: `${invocation.id}:${workflowNode.id}`, position: isRoot ? workflowNode.position : { x: offsetX + workflowNode.position.x - minX, y: offsetY + 115 + workflowNode.position.y - minY }, sourcePosition: Position.Right, targetPosition: Position.Left, data: { label: <RunNodeLabel type={workflowNode.type} label={workflowNode.label} status={execution?.status ?? "PENDING"} /> }, className: `status-${(execution?.status ?? "PENDING").toLowerCase()}`, draggable: false });
    }
