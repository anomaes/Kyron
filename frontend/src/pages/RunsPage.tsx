...
export function RunsPage() {
  ...
  return <section><header className="page-header"><div><p className="eyebrow">Execution history</p><h1>Runs</h1><p>Every run is pinned to an exact commit and preserves its attempt history.</p></div></header>
    {runs.data?.items.length ? <div className="table-card"><table><thead><tr><th>Status</th><th>Workflow</th><th>Base revision</th><th>Triggered by</th><th>Started</th><th>Current</th><th>Review</th></tr></thead><tbody>{runs.data.items.map((run) => <tr key={run.id}><td><StatusBadge status={run.status} /></td><td><Link to={`/runs/${run.id}`}><strong>{run.root_workflow_id}</strong>{run.local_definition_test && <small className="block">Local test</small>}<small className="mono block">{run.id.slice(0, 8)}</small></Link></td><td><span>{run.base_ref}</span><code className="block">{run.base_commit_sha.slice(0, 10)}</code></td><td>@{run.reviewer_provider_username}</td><td>{run.started_at ? new Date(run.started_at).toLocaleString() : "Queued"}</td><td className="mono">{run.current_node_execution_id?.slice(0, 8) ?? "—"}</td><td>{run.change_request_url ? <a href={run.change_request_url} target="_blank" rel="noreferrer">{run.reviewer_provider === "gitlab" ? "!" : "#"}{run.change_request_number}</a> : run.local_definition_test ? "Local only" : "—"}</td></tr>)}</tbody></table></div> : <EmptyState title="No workflow runs yet">Trigger a workflow from its project catalog.</EmptyState>}
  </section>;
}
