...
function FolderTree({ folder, renderRows, collapsedFolders, onToggle, root = false }: { folder: WorkflowFolder; renderRows: (items: WorkflowListItem[]) => ReactNode; collapsedFolders: Set<string>; onToggle: (path: string) => void; root?: boolean }) {
  const children = Array.from(folder.children.values()).sort((left, right) => left.name.localeCompare(right.name));
  const folderKey = root ? "/" : folder.path;
  const isOpen = !collapsedFolders.has(folderKey);
  const count = folderWorkflowCount(folder);
  return <section className={root ? "workflow-folder-root" : "workflow-folder-branch"}><header className="workflow-folder-title"><button type="button" className="workflow-folder-toggle" aria-expanded={isOpen} onClick={() => onToggle(folderKey)}><span className="workflow-folder-chevron" aria-hidden="true">›</span><span>{folder.name}{root && "/"}</span></button><small>{count} workflow{count === 1 ? "" : "s"}</small></header>{isOpen && <div className="workflow-folder-contents">{folder.items.length > 0 && renderRows(folder.items)}{children.map((child) => <FolderTree key={child.path} folder={child} renderRows={renderRows} collapsedFolders={collapsedFolders} onToggle={onToggle} />)}</div>}</section>;
}
...
export function WorkflowListPage() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const subject = data.get("subject_type") === "change_request"
      ? { type: "change_request", number: Number(data.get("change_request_number")) }
      : { type: "branch", ref: String(data.get("base_ref") || project.data?.default_branch || "main") };
    trigger.mutate({ workflow: selected.id, body: { subject, inputs, use_local_definitions: data.get("use_local_definitions") === "on" } });
  }
