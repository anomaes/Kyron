...
export function ProjectsPage() {
  ...
  const updateProjectToken = useMutation({
    mutationFn: ({ projectId, access_token }: { projectId: string; access_token: string }) => api<Project>(`/projects/${projectId}/token`, json("PUT", { access_token })),
    onSuccess: () => { setTokenTarget(null); void queryClient.invalidateQueries({ queryKey: ["projects"] }); },
  });
  ...
  function submitProjectToken(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!tokenTarget) return;
    const data = new FormData(event.currentTarget);
    updateProjectToken.mutate({
      projectId: tokenTarget.id,
  ...
  return (
    <section>
      ...
      {tokenTarget && <div className="modal-backdrop" onMouseDown={() => setTokenTarget(null)}><form className="modal" onSubmit={submitProjectToken} onMouseDown={(event) => event.stopPropagation()}><h2>Access token for {tokenTarget.name}</h2><p className="muted">Kyron validates the replacement against {tokenTarget.provider === "github" ? "GitHub" : "GitLab"} before storing it. The existing token cannot be retrieved.</p><label>New project access token<input name="access_token" type="password" required autoComplete="new-password" /></label>{updateProjectToken.error && <p className="error" role="alert">{updateProjectToken.error.message}</p>}<footer><button type="button" className="secondary" onClick={() => setTokenTarget(null)}>Cancel</button><button disabled={updateProjectToken.isPending}>{updateProjectToken.isPending ? "Validating…" : "Validate & replace"}</button></footer></form></div>}
