...
```yaml
prompt: Implement ${TASK}. Inspect the repository first, keep changes scoped, and
  run relevant tests.
```
...
```yaml
prompt: |-
  Revise the current implementation for this task: ${ORIGINAL_TASK}
...
- [ ] No token, credential, or authenticated URL appears in YAML.
...
For every field and constraint, use the [complete authoring specification](/workflow-yaml-authoring-spec).
